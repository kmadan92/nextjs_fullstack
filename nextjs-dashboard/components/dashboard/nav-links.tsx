...
export default function NavLinks() {
  ...
  // 1. Determine which list to use
  const currentLinks = pathname === "/" ? linksOnDashboard : links;

  // 2. Return the mapped list
  return (
    <>
      {currentLinks.map((link) => {
        ...
        return (
          <Link
            key={link.name} // Key is now on the outermost element
            href={link.href}
            className={clsx(
